...
with tf.Session(graph=graph) as sess:
    ...
    Q = QNetwork(sess, inputLength, outputLength, args.hidden_layers_state_value, obsPh, aPh, suffix="orig") # original Q network
    QAux = QNetwork(sess, inputLength, outputLength, args.hidden_layers_state_value, obsPh, policy.actionFinal,suffix="aux", reuse=Q) # network with parameters same as original Q network, but instead of action placeholder it takse output from current policy
    QTarget = QNetwork(sess, inputLength, outputLength, args.hidden_layers_state_value, nextObsPh, policyTarget.actionFinal,suffix="target") #target Q network, instead of action placeholder it takse output from target policy
    ...
    while step < args.total_train_steps:  
        ...
        while doSample:              
            ...
            if step % args.update_freq == 0:           
                ...
                QTarget.polyak(Q, args.rho)
                policyTarget.polyak(policy, args.rho)       
